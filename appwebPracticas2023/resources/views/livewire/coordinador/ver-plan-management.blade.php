        ...
        <div
            ...
            <div class="rounded overflow-hidden shadow-lg p-6 bg-white dark:bg-gray-800 border-2 ">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($formatoplans as $item)
                        <div
                            class="rounded overflow-hidden shadow-lg p-6 bg-white dark:bg-gray-800 border-4 hover:shadow-2xl transition duration-500">
                            <div class="font-bold text-xl mb-2">Plan Numero {{ $item->id }}</div>
                            <p>Codigo: {{ $item->student ? $item->student->code : 'No hay Estudiante' }}</p>
                            <p>Nombre del Plan: {{ $item->student ? $item->student->name : 'No hay Estudiante' }}</p>
                            ...
                            <form>
                                <div class="mt-4" id="estadoDiv{{ $item->id }}" style="display: none;">
                                    ...
                                    <button type="button" onclick="ocultarEstado({{ $item->id }})"
                                        class="px-4 py-2 bg-orange-500 text-white rounded mt-4">
